...
class LeadStrikerDecision : public SyncActionNode
{
public: 
    LeadStrikerDecision(const string &name, const NodeConfig &config, Brain *_brain) : SyncActionNode(name, config), brain(_brain) {}

    static PortsList providedPorts()
    {
        return {
            InputPort<double>("chase_threshold", 1.0, "이 거리보다 멀어지면 공을 추격하는 동작을 수행"),
            InputPort<double>("set_piece_goal_dist", 1.5, "이 거리 이내에서는 정렬 생략하고 One Touch 슛 시도"),
            InputPort<double>("kick_tolerance", 0.3, "킥 결정을 위한 각도 허용 오차 (rad)"),
            InputPort<double>("yaw_tolerance", 0.5, "킥 결정을 위한 ball yaw 허용 오차 (rad)"),
            InputPort<string>("decision_in", "", "이전 틱에서의 decision 값을 읽기 위해 사용"),
            OutputPort<string>("decision_out")};
...
class SubStrikerDecision : public SyncActionNode
{
public: 
    SubStrikerDecision(const string &name, const NodeConfig &config, Brain *_brain) : SyncActionNode(name, config), brain(_brain) {}
...
class StrikerScreening : public SyncActionNode
{
public:
    StrikerScreening(const string &name, const NodeConfig &config, Brain *_brain)
