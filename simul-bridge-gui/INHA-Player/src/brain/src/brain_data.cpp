...
void BrainData::updateRobots(const vector<GameObject>& newObservations, double retentionTime) {
    ...
    const double alpha = 0.3; // Opponent 위치 EMA 계수
    ...
    for (const auto& newObj : newObservations) {
        ...
        if (bestIdx != -1) {
            if (newObj.label == "Opponent" && _robots[bestIdx].label == "Opponent") {
                double emaX = alpha * newObj.posToField.x + (1.0 - alpha) * _robots[bestIdx].posToField.x;
                double emaY = alpha * newObj.posToField.y + (1.0 - alpha) * _robots[bestIdx].posToField.y;

                _robots[bestIdx] = newObj; // 최신 메타/타임스탬프 유지
                _robots[bestIdx].posToField.x = emaX;
                _robots[bestIdx].posToField.y = emaY;

                Pose2D pf{emaX, emaY, 0};
                Pose2D pr = field2robot(pf);
                _robots[bestIdx].posToRobot.x = pr.x;
                _robots[bestIdx].posToRobot.y = pr.y;
                _robots[bestIdx].range = norm(pr.x, pr.y);
                _robots[bestIdx].yawToRobot = atan2(pr.y, pr.x);
