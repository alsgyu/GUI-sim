...
void Brain::init(){   
    ...
    gtPoseSubscription = create_subscription<geometry_msgs::msg::PoseStamped>("gt_pose", SUB_STATE_QUEUE_SIZE, bind(&Brain::gtPoseCallback, this, _1));
...
// 함수 추    
  void Brain::gtPoseCallback(const geometry_msgs::msg::PoseStamped &msg){
    // 시뮬레이션 GT 위치를 직접 field 좌표로 사용
...
void Brain::gameControlCallback(const game_controller_interface::msg::GameControlData &msg){
    ...
    tree->setEntry<bool>("gc_is_our_kickoff", isKickOffSide); // 적용
    ...
    switch (static_cast<int>(msg.secondary_state)) {
        case 0:
            ...
            break;
        case 1:
            gameSubStateType = "SHOOT_OUT";
            data->realGameSubState = "SHOOT_OUT";
            break;
        case 2:
            gameSubStateType = "EXTRA_TIME";
            data->realGameSubState = "EXTRA_TIME";
            break;
        case 3:
    ...
    tree->setEntry<string>("gc_game_secondary_state", gameSubStateType); // 현재 어떤 프리킥 상황인지
    tree->setEntry<string>("gc_game_secondary_state_info", gameSubState); // 정지, 공격 또는 수비 위치로 이동, 대기 자세인지 확인

    // 프리킥 상황에서 우리 팀이 선공(킥 수행) 팀인지 여부. 
    bool isSubStateKickOffSide = (static_cast<int>(msg.secondary_state_info[0]) == config->teamId); 
    tree->setEntry<bool>("gc_is_our_secondary_state", isSubStateKickOffSide);

    // 추가: 이전 setpiece 저장
    // gameSubStateType 계산 완료 이후에 추가
    ...
    const bool isSetpiecePhase = (gameSubState == "STOP" || gameSubState == "GET_READY" || gameSubState == "SET");

    if (!isExcluded & isSetpiecePhase) {
        tree->setEntry<string>("gc_last_setpiece_type", gameSubStateType);
    }
    ...
    // 잔여 시간
    data->secsRemaining = static_cast<int>(msg.secs_remaining); // find more details in game_controller/src/game_controller_node.cpp/handle_packet
}
...
void Brain::detectionsCallback(const vision_interface::msg::Detections &msg){
    ...
    detection_utils::detectProcessMarkings(markings, data, config, log, locator, this);
...
bool Brain::isFreekickStartPlacing() {
    return (tree->getEntry<string>("gc_game_secondary_state") == "FREE_KICK" && tree->getEntry<string>("gc_game_state") == "PLAY" && tree->getEntry<string>("gc_game_secondary_state_info") == "GET_READY");
}
// 장애물 리스트 업데이트 함수
// 로그는 주석 처리됨
...
void Brain::calibrateOdom(double x, double y, double theta){
    ...
    if (data->ballDetected) {
        ...
        // 미관측 시에는 field 좌표를 유지하고, 상대좌표만 갱신
        updateRelativePos(data->ball);
    }

    // 어떤 경우든 robot->ball 방향은 최신화
    data->robotBallAngleToField = atan2(
        data->ball.posToField.y - data->robotPoseToField.y,
        data->ball.posToField.x - data->robotPoseToField.x
    );
...
void Brain::handleCooperation() {
    ...
    TMStatus& selfStatus = data->tmStatus[selfIdx];
    ...
    // [ADD] self 상태를 tmStatus[selfIdx]에 매 tick 반영
    {
        const auto nowCom = get_clock()->now();
        selfStatus.role = tree->getEntry<string>("player_role");
        selfStatus.isAlive = (data->penalty[selfIdx] == PENALTY_NONE);  // 필요시 penalty 기준으로 분리 가능
        selfStatus.ballDetected = data->ballDetected;
        selfStatus.ballLocationKnown = tree->getEntry<bool>("ball_location_known");
        selfStatus.ballConfidence = data->ball.confidence;
        selfStatus.ballRange = data->ball.range;
        selfStatus.ballPosToField = data->ball.posToField;
        selfStatus.robotPoseToField = data->robotPoseToField;
        selfStatus.kickDir = data->kickDir;
        selfStatus.thetaRb = data->robotBallAngleToField;
        selfStatus.cost = data->tmMyCost;
        selfStatus.timeLastCom = nowCom;
    }
    ...
    if (trustedTMIdx >= 0) {   
        ...
        // [A] 내 관측이 없으면 팀 관측으로 즉시 갱신 (ball_location_known 여부와 무관)
        if (!data->ballDetected) {
            data->ball.posToField = data->tmBall.posToField;
            data->ball.timePoint = get_clock()->now(); // memory timeout 기준시각 갱신
    ...
    //=====isPossession=====
    {
        const auto &posBall = data->ball.posToField;
        auto posD2 = [&](double x, double y) {
            x -= posBall.x;
            y -= posBall.y;
            return x * x + y * y;
        };

        double posMyMinD2 = 1e12;
        if (data->tmImAlive) posMyMinD2 = posD2(data->robotPoseToField.x, data->robotPoseToField.y);
        for (int i = 0; i < aliveTmIdxs.size(); ++i) {
            const auto &posTm = data->tmStatus[aliveTmIdxs[i]].robotPoseToField;
            posMyMinD2 = std::min(posMyMinD2, posD2(posTm.x, posTm.y));
        }

        double posOppMinD2 = 1e12;
        const auto posSeenRobots = data->getRobots();
        for (int i = 0; i < posSeenRobots.size(); ++i) {
            if (posSeenRobots[i].label != "Opponent") continue;
            posOppMinD2 = std::min(posOppMinD2, posD2(posSeenRobots[i].posToField.x, posSeenRobots[i].posToField.y));
        }

        selfStatus.isRace = (posMyMinD2 < 1.0 && posOppMinD2 < 1.0);
        selfStatus.isPossession = (posMyMinD2 <= posOppMinD2);
    ...
    // [ADD] 리더 판정 결과를 selfStatus에도 반영
    selfStatus.isLead = data->tmImLead;
    selfStatus.cost = data->tmMyCost;
    selfStatus.timeLastCom = get_clock()->now();
    ...
    if (
        (tree->getEntry<string>("gc_game_state") == "READY" || tree->getEntry<string>("gc_game_secondary_state_info") == "GET_READY") 
...
void Brain::updateKickoffMemory() {
    ...
    bool isWaitingForKickoff = (
        ...
        && !tree->getEntry<bool>("gc_is_our_kickoff")
    );
    bool isWaitingForFreekickKickoff = (
        (tree->getEntry<string>("gc_game_secondary_state_info") == "SET" || tree->getEntry<string>("gc_game_secondary_state_info") == "GET_READY")
        && !tree->getEntry<bool>("gc_is_our_secondary_state")
    );
