    ...
    for (int i = 0; i < mapLines.size(); i++) { // 모든 기준 라인과 비교
        ...
        if (mapLine.type == LineType::GoalLine) {  // 골라인의 경우
            ...
            if (
                ...
                && (tree->getEntry<string>("gc_game_secondary_state_info") == "GET_READY" || tree->getEntry<string>("gc_game_secondary_state_info") == "SET") // 준비 상태이면서
        ...
        if (mapLine.type == LineType::MiddleLine) { // 중앙선의 경우
            ...
            if (
                ...
                && (tree->getEntry<string>("gc_game_secondary_state_info") == "GET_READY" || tree->getEntry<string>("gc_game_secondary_state_info") == "SET")
...
void detectProcessMarkings(const vector<GameObject> &markingObjs, const std::shared_ptr<BrainData> &data, const std::shared_ptr<BrainConfig> &config, const std::shared_ptr<BrainLog> &log, const std::shared_ptr<Locator> &locator, Brain* brain){
    ...
    //     brain->calibrateOdom(est.x, est.y, est.theta);
