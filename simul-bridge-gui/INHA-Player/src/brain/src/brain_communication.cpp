...
void BrainCommunication::initCommunication()
{
    ...
    if (brain->config->enableCom)
    {
        ...
        _unicast_udp_port = 30000 + brain->config->teamId * 100 + brain->config->playerId;
        ...
        // 같은 PC에서 실행 시 broadcast discovery가 불안정하므로,
        // localhost(127.0.0.1)로 모든 팀원을 미리 등록
    
        std::lock_guard<std::mutex> lock(_teammate_addresses_mutex);
        uint32_t loopback_ip = inet_addr("127.0.0.1");
        auto now = brain->get_clock()->now();
        for (int pid = 1; pid <= brain->config->numOfPlayers; pid++) {
            if (pid == brain->config->playerId) continue;
            _teammate_addresses[pid] = { loopback_ip, pid, now };
            cout << GREEN_CODE << format("Pre-registered teammate player %d on localhost", pid)
                    << RESET_CODE << endl;
        }
    }
    else
...
void BrainCommunication::initDiscoveryReceiver()
{
    try
    {
        ...
        // 같은 PC에서 여러 프로세스가 동일 포트로 broadcast 수신 가능하도록
        if (setsockopt(_discovery_recv_socket, SOL_SOCKET, SO_REUSEPORT, &reuse, sizeof(reuse)) < 0)
        {
            cout << RED_CODE << format("Failed to set SO_REUSEPORT: %s", strerror(errno))
                << RESET_CODE << endl;
            throw std::runtime_error(strerror(errno));
        }
...
void BrainCommunication::spinDiscoveryReceiver() {    
    ...
    while (_receive_discovery_flag) {
        ...
        if (msg.playerId == brain->config->playerId) { 
            ...
            std::lock_guard<std::mutex> lock(_teammate_addresses_mutex);
            // playerId 기반 키 사용 (같은 PC에서 IP가 동일해도 구분 가능)
            _teammate_addresses[msg.playerId] = {
...
void BrainCommunication::unicastCommunication() {
    ...
    while (_unicast_communication_flag) {
        ...
        TeamCommunicationMsg msg{}; // editted from msg;
        msg.validation = VALIDATION_COMMUNICATION;
        msg.communicationId = _team_communication_msg_id++;
        msg.teamId = brain->config->teamId;
        msg.playerId = brain->config->playerId;
        msg.playerRole = brain->tree->getEntry<string>("player_role") == "field_player" ? 1 : 2;
        ...
        msg.passSignal = brain->data->tmStatus[brain->config->playerId-1].passSignal;
        msg.passTargetX = brain->data->tmStatus[brain->config->playerId-1].passTargetX;
        msg.passTargetY = brain->data->tmStatus[brain->config->playerId-1].passTargetY;
        msg.isRace = brain->data->tmStatus[brain->config->playerId-1].isRace;
        msg.isPossession = brain->data->tmStatus[brain->config->playerId-1].isPossession;
        //=====Robot Sharing=====
        msg.sharedRobotCount = 0;
        const auto seenRobots = brain->data->getRobots();
        for (int i = 0; i < (int)seenRobots.size() && msg.sharedRobotCount < MAX_SHARED_ROBOTS; ++i) {
            const auto& r = seenRobots[i];
            if (r.label != "Opponent") continue; // 우선 상대만 공유

            auto& out = msg.sharedRobots[msg.sharedRobotCount++];
            out.x = static_cast<float>(r.posToField.x);
            out.y = static_cast<float>(r.posToField.y);
            out.confidence = static_cast<float>(r.confidence);
            out.label = SHARED_LABEL_OPPONENT;
        }
        //========================
        ...
        for (auto it = _teammate_addresses.begin(); it != _teammate_addresses.end(); ++it) {
            ...
            // 같은 PC 대응: 팀원별 고유 포트로 전송
            int target_port = 30000 + brain->config->teamId * 100 + it->second.playerId;
            _unicast_saddr.sin_addr.s_addr = ip;
            _unicast_saddr.sin_port = htons(target_port);
...
void BrainCommunication::spinCommunicationReceiver() {
    ...
    while (_receive_communication_flag) {
        ...
        // unicast 수신 시에도 teammate lastUpdate 갱신 (pre-registered 팀원 timeout 방지)
        {
            std::lock_guard<std::mutex> lock(_teammate_addresses_mutex);
            auto it = _teammate_addresses.find(msg.playerId);
        ...
        tmStatus.role = msg.playerRole == 1 ? "field_player" : msg.playerRole == 2 ? "goal_keeper" : "unknown"; // 역할
        ...
        tmStatus.isRace = msg.isRace; // 경합상황
        tmStatus.isPossession = msg.isPossession; // 볼 점유 여부

        //=====Robot Sharing=====
        std::vector<GameObject> sharedRobots;
        sharedRobots.reserve(msg.sharedRobotCount);

        const auto recvNow = brain->get_clock()->now();
        for (int i = 0; i < msg.sharedRobotCount && i < MAX_SHARED_ROBOTS; ++i) {
            const auto& in = msg.sharedRobots[i];
            if (in.label != SHARED_LABEL_OPPONENT) continue;
            ...
            Pose2D pf{r.posToField.x, r.posToField.y, 0.0};
            Pose2D pr = brain->data->field2robot(pf);
            r.posToRobot.x = pr.x;
            r.posToRobot.y = pr.y;
            r.posToRobot.z = 0.0;

            sharedRobots.push_back(r);
        }

        if (!sharedRobots.empty()) {
            brain->data->updateRobots(sharedRobots, 1.0);
        }
        //=======================
