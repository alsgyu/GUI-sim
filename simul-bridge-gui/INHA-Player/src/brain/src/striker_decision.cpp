...
void RegisterStrikerDecisionNodes(BT::BehaviorTreeFactory &factory, Brain* brain){
    ...
    REGISTER_STRIKERDECISION_BUILDER(LeadStrikerDecision)
    REGISTER_STRIKERDECISION_BUILDER(SubStrikerDecision)
    REGISTER_STRIKERDECISION_BUILDER(StrikerScreening)
}
...
NodeStatus LeadStrikerDecision::tick() {
    // --- 전술(Tactic)에서 설정한 파라미터를 읽어오기 ---
    double paramChaseSpeed = 0.8;
    double paramDefenseLineX = -10.0; // 기본값: 제한 없음
    double paramKickThreshold = 0.3;  
    ...
    // 공이 적팀 골대 안으로 들어갔는지 판별
    // x가 골라인보다 크고(즉, 넘어갔고), y가 골대 폭 안에 있으면 골로 인정
    if (ball.posToField.x > goalX + 0.1 && fabs(ball.posToField.y) < goalY) {
        brain->data->hasScored = true;
    }
    else brain->data->hasScored = false;
    // 골을 넣었다면 멈추기
    if (brain->data->hasScored) {
        setOutput("decision_out", string("stop_goal"));
        brain->log->logToScreen("tree/Decide", "GOAL SCORED! Stopping...", 0xFF0000FF);
    ...
    // TODO: 세트피스시, 적당한 플래그를 넘겨받아 무조건 슛시퀀스를 진행하도록 해야
    if (!(iKnowBallPos || tmBallPosReliable)) {
        newDecision = "find"; // 1. 공 찾기
        color = 0xFFFFFFFF;
    }
    else if (ballRange > chaseRangeThreshold) {
        newDecision = "chase"; // 2. chase
        color = 0x0000FFFF;
    } 
    else if (distToGoal > 3.5 && !passFound){
        newDecision = "dribble"; // 4. 드리블
        color = 0x0000FFFF;
    }
    else { // 5. 슛 시퀀스 -> 이후 '비전기반 킥'으로 대체필요
        // getInput("kick_tolerance", kickTolerance); // xml에서 수정 가능하도록
        // getInput("yaw_tolerance", yawTolerance);
        double kickTolerance = paramKickThreshold; 
        double yawTolerance = 0.5; // Yaw는 넉넉하게
        ...
        if (
            ...
            else if (distToGoal < setPieceGoalDist) newDecision = "kick_quick"; 
            else newDecision = "kick";      
    ...
    brain->log->logToScreen(
        "tree/Decide",
        format(
            "LeadStrikerDecision: %s ballrange: %.2f ballyaw: %.2f", 
...
NodeStatus SubStrikerDecision::tick() {
    // --- 전술(Tactic)에서 설정한 파라미터를 읽어오기 ---
    double paramChaseSpeed = 0.8;
    double paramDefenseLineX = -10.0; // 기본값: 제한 없음
    double paramKickThreshold = 0.3;  
    ...
    if (!(iKnowBallPos || tmBallPosReliable)) {
        ...
        newDecision = "receive"; // 2. 패스받으러 이동
        color = 0x00FFFFFF;
    }
    else if (distToGoal < 2.0){
        newDecision = "screen"; // 3. 스크리닝
        color = 0xFFFF00FF;
    }
    else {
        newDecision = "offtheball"; // 4. 오프더볼
        color = 0x0000FFFF;
    }
    ...
    brain->log->logToScreen(
        "tree/Decide",
        format(
            "SubStrikerDecision: %s ballrange: %.2f ballyaw: %.2f", 
            newDecision.c_str(), ballRange, ballYaw
        ),
        color
    );
...
NodeStatus StrikerScreening::tick() {
    auto pose = brain->data->robotPoseToField;
    ...
    int targetOppIdx = nearestIdx;
    const char* usedRank = (targetOppIdx >= 0) ? "first" : "none";
    ...
    if (targetOppIdx >= 0) {
        ...
        // 공(B)~상대(O) 선분에서 BP:PO = 2:1 (상대 쪽에 더 가까움)
        tx = (bx + 2.0 * ox) / 3.0;
        ty = (by + 2.0 * oy) / 3.0;

        brain->log->setTimeNow();
        brain->log->log(
            "field/striker_screening_dir",
    ...
    // 도착 시 정지
    if (dist < stopThreshold || targetOppIdx < 0) {
        controlx = 0.0;
        controly = 0.0;
        vtheta = 0.0;
    }
    ...
    brain->log->setTimeNow();
    brain->log->log(
        "field/striker_screening_target",
        rerun::Points2D({{(float)tx, -(float)ty}})
            .with_colors({0x00FFFFFF})
            .with_radii(0.05f)
    );
    brain->log->logToScreen(
        "tree/StrikerScreening",
        format("Screening target=(%.2f, %.2f) oppCount=%d use=%s", tx, ty, validOpponentCount, usedRank),
        0x00FFFFFF
    );
