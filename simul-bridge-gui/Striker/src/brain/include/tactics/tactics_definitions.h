...
class TacticFinishing : public BT::SyncActionNode
{
public:
    TacticFinishing(const string &name, const NodeConfig &config, Brain *_brain) : BT::SyncActionNode(name, config), brain(_brain) {}
    static PortsList providedPorts();
    NodeStatus tick() override;
private:
    Brain *brain;
...
// 전략 7. 텐백 수비 (Deep Defense / Park the Bus)
